...
-(void)startPlayingLocalFileWithName:(NSString *)name atPath:(NSString *)path withCompletionBlock:(progressBlock)block {
    ...
    NSString *defaultPath = [NSString stringWithFormat:@"%@%@", [[[NSFileManager defaultManager] URLsForDirectory:NSDocumentDirectory inDomains:NSUserDomainMask] lastObject], name];
    NSURL *fileURL = [NSURL URLWithString:defaultPath];
    ...
    if (!error) {
        ...
        _timer = [NSTimer scheduledTimerWithTimeInterval:0.5 block:^{
            ...
            if ((CMTimeGetSeconds(_player.currentItem.duration) - CMTimeGetSeconds(_player.currentItem.currentTime)) != 0) {
                ...
                NSLog(@"%f,%d",CMTimeGetSeconds(_player.currentItem.currentTime),timeRemaining);
                if (CMTimeGetSeconds(_player.currentItem.currentTime) >=  1){
                    self.appdel = (AppDelegate *)[[UIApplication sharedApplication] delegate];
                    self.appdel.objCustomTabBar.                   customTabobj.btnSkip.enabled = true;
                }
                /*if (CMTimeGetSeconds(_player.currentItem.currentTime) < 10) {
                    _player.volume = vlm;
                    vlm += 0.1;
                }
